...
/**
 * clearSessionCookie(opts?)
 *
 * Returns the Set-Cookie header string that expires the session cookie.
 * opts.secure (default true) — set to false in local http dev to omit Secure,
 * matching the flag used when the cookie was set by serializeSessionCookie /
 * login.  A browser will only accept a cookie deletion when the Secure
 * attribute matches what was set; on plain-http localhost Secure is never set,
 * so omitting it here prevents a silent no-op logout.
 * Max-Age=0 is the reliable cross-browser way to delete a cookie.
 */
...
export function checkSameOrigin(req) {
  // Derive the server host from req.headers['host'] only.
  // We deliberately do NOT use x-forwarded-host here: that header is
  // client-suppliable (e.g. via a reverse-proxy misconfiguration or a crafted
  // request), and trusting it would let an attacker forge a host match.
  // On Vercel, the canonical host for same-origin checks is the `host` header.
